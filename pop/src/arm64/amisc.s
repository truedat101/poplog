...
DEF_C_LAB (_iscompound)
    ...
    b.ne .Lcomp_false
    adrp x0, .L.true
    add  x0, x0, :lo12:.L.true
    ldr  x0, [x0]
    str  x0, [USP]
    ret
.Lcomp_false:
    adrp x0, .L.false
    add  x0, x0, :lo12:.L.false
    ldr  x0, [x0]
    str  x0, [USP]
    ret
...
DEF_C_LAB (_neg)
    ...
    b.ge .Lneg_false
    adrp W0, .L.true
    add  W0, W0, :lo12:.L.true
    ldr  W0, [W0]
    str  W0, [USP]
    ret
.Lneg_false:
    adrp W0, .L.false
    add  W0, W0, :lo12:.L.false
    ldr  W0, [W0]
    str  W0, [USP]
    ret
...
DEF_C_LAB (_checkplogall)
    adrp x0, C_LAB(_special_var_block)
    add  x0, x0, :lo12:C_LAB(_special_var_block)
...
.L11.check_interrupt:
    ...
    tst  x1, #1
    b.eq .L11.ret
    adrp x1, I_LAB(_disable)
    add  x1, x1, :lo12:I_LAB(_disable)
    ldr  x1, [x1]
    tst  x1, #1
    b.eq XC_LAB(Sys$-Async_raise_signal)
.L11.ret:
    ret
...
.L11.do_user_overflow:
    ...
    ldr  x1, [x1]
    tst  x1, #2
    b.eq XC_LAB(Sys$-User_overflow)
    b    .L11.check_interrupt

DEF_C_LAB (_checkinterrupt)
    adrp x1, I_LAB(_trap)
    add  x1, x1, :lo12:I_LAB(_trap)
    ldr  x1, [x1]
    tst  x1, #1
    b.eq .L12.ret
    b    C_LAB (_checkall)
...
EXTERN_NAME(pop_print):
    ...
    adrp PB, .L11.dummy_procedure_helper
    add  PB, PB, :lo12:.L11.dummy_procedure_helper
    ldr  PB, [PB]
    str  PB, [sp, #-16]!
